...
export default function ModificarEmpleadoAdmin({ route, navigation }) {
  ...
  const modificarEmpleado = async () => {
   ...
   const eliminar = async() => {
    Alert.alert('Trivago', "¿Esta seguro que desea eliminar este empleado?", [
                
      { text: 'Si', onPress: () =>  {DesactivarEmpleado()}},
      {text : 'No', onPress: () =>{}}
  ...
  const DesactivarEmpleado = async () => {
        ...
        try {
            ...
            const desactivarEmpleado = await fetch(
              'http://' + global.ip + ':4002/api/empleados/desactivar?id=' + idEmpleado, {
              method: 'PUT',
              headers: {
                Accept: 'application/json',
                'Content-Type': 'application/json',
            ...
            const json2 = await desactivarEmpleado.json();
