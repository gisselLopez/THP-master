...
export default function modificarClienteEmpleado({route, navigation }) {
  let idCliente = route.params.id;
  useEffect(() => {
  ...
  const modificarCliente = async () => {
        ...
        if (json.data) {
          ...
          if (json2.data) {
            ...
            Alert.alert('Trivago', 'Registros guardados', [
              ...
              { text: 'OK', onPress: () => { navigation.navigate('ListaCliente'); } },
            ])
          }
   ...
   const eliminar = async() => {
    Alert.alert('Trivago', "¿Esta seguro que desea eliminar este cliente?", [
                
      { text: 'Si', onPress: () =>  {DesactivarCliente()}},
      {text : 'No', onPress: () =>{}}
  ...
  const DesactivarCliente = async () => {
       var idCliente = JSON.parse(await AsyncStorage.getItem('Cliente'));
      if (!identidad || !nombres || !apellidos || !telefono || !correo || !direccion || !usuario ||  !estado || !contrasenia ) {
        console.log("Escriba los datos completos");
        Alert.alert("Trivago", "Escriba los datos completos");
      }
      else {
        try {
            ...
            const desactivarCliente = await fetch(
              'http://' + global.ip + ':4002/api/clientes/desactivar?id=' + idCliente, {
              method: 'PUT',
              headers: {
                Accept: 'application/json',
                'Content-Type': 'application/json',
            ...
            const json2 = await desactivarCliente.json();
      ...
      <ScrollView>
        <BlurView intensity={30}>
          <View style={styles.clientes}>
            ...
            <TouchableOpacity  onPress={eliminar} style={[styles.button, { backgroundColor: '#FF0000' }]}>
                        <Text style={{ fontSize: 20, fontWeight: 'bold', color: 'white' }}>Eliminar</Text>
            </TouchableOpacity>
          </View>
        </BlurView>
      </ScrollView>
